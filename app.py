...
def pedido_soporte():
    # Se agregan equipos
    # 4 letras con 4 numeros
    id_equipo = request.GET.get('id_equipo','')
    # 2 numeros
    falla = request.GET.get('tipo_falla','')
    conn = sqlite3.connect('pedidos_soporte.db')

    #Se inserta los atributos a la tabla en la Base de Datos
    query = f"""INSERT INTO
                fallas (descripcion)
                VALUES ('{falla}');"""
...
#URL siguiente equipo a arreglar    
@route('/siguiente')
